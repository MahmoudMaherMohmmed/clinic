        ...
        <div class="tab-content">
            ...
            @foreach ($languages as $language)
                <div class="tab-pane fade in {{ $i++ ? '' : 'active' }}" id="title{{ $language->short_code }}">
                    <input class="form-control" name="title[{{ $language->short_code }}]" value="@if ($new) {!! $new->getTranslation('title', $language->short_code) !!} @endif" />
                </div>
            @endforeach
        </div>
    </div>
            ...
            @foreach ($languages as $language)
                <div class="tab-pane fade in {{ $i++ ? '' : 'active' }}"
                    ...
                    <textarea class="form-control col-md-12"
                        ...
                        rows="6"> {{ isset($new)&&$new!=null ? $new->getTranslation('description', $language->short_code) : old('description.' . $language->short_code)}}
                    </textarea>
                </div>
            @endforeach
        </div>
